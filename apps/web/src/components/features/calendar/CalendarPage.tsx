...
import { MonthCalendar, useMonthCalendar } from '@goal-chaser/sdk'
import type { DayCustomization, AIPreviewData } from '@goal-chaser/sdk'
...
export default function CalendarPage({
  ...
  useEffect(() => {
    ...
    const loadFilters = async () => {
      try {
        ...
        if (saved) {
          ...
          } else if (enabledAddons?.includes('productivity')) {
            setBackgroundSource('productivity')
          }
        } else if (enabledAddons?.includes('productivity')) {
          setBackgroundSource('productivity')
        }
    ...
    loadFilters()
  }, [user?.uid, goalId, enabledAddons])

  // Default background to productivity when addons load after filters
  useEffect(() => {
    if (
      filtersLoaded &&
  ...
  }, [filtersLoaded, backgroundSource, enabledAddons])
  ...
  const handleAIFill = async (
    ...
    try {
      ...
      const response = await fetch('/api/plugin-action', {
        method: 'POST',
      ...
      const preview: AIPreviewData[] = aiEnabledPlugins.map((pluginId) => {
        ...
        const result = data.results.find(
          (r: { pluginId: string }) => r.pluginId === pluginId,
        )
  ...
  const handleUpdatePluginConfig = useCallback(
    async (pluginId: string, configUpdate: Record<string, unknown>) => {
  ...
  return (
    <div className="space-y-6">
      ...
      <AIWorkflowWizard
        ...
        existingDayData={
          selectedDate ? pluginDataByDate[selectedDate] || {} : {}
        }
