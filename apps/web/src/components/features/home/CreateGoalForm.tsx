...
export function CreateGoalForm({ onSubmit, onCancel }: CreateGoalFormProps) {
  ...
  const [selectedPlugins, setSelectedPlugins] = useState<AddonId[]>([])
  
  const { plugins: availablePlugins, loading: pluginsLoading } = useManageablePlugins()

  const togglePlugin = (pluginId: AddonId) => {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (name.trim()) {
      onSubmit({
        ...
        enabledPlugins: selectedPlugins.length > 0 ? selectedPlugins : availablePlugins.map(p => p.id as AddonId),
      })
    }
  }
    ...
    <form onSubmit={handleSubmit} className="space-y-5">
      ...
      <div>
        <label className="block text-sm font-medium text-white/60 mb-3">
          Enable Plugins {selectedPlugins.length === 0 && <span className="text-white/40">(All plugins enabled by default)</span>}
        </label>
        {pluginsLoading ? (
          <div className="text-center py-8 text-white/40">Loading plugins...</div>
        ) : (
          <div className="grid grid-cols-2 gap-3">
            {availablePlugins.map((plugin) => {
              const isSelected = selectedPlugins.includes(plugin.id as AddonId)
              return (
                <button
                  key={plugin.id}
                  type="button"
                  onClick={() => togglePlugin(plugin.id as AddonId)}
                  className={`
                ...
                >
                  <div className="flex items-center gap-2 mb-1.5">
                    ...
                    <span className={`font-medium ${isSelected ? 'text-[#30D158]' : 'text-white/80'}`}>
        ...
        <p className="text-xs text-white/30 mt-3">
          {selectedPlugins.length > 0 
            ...
            : 'All plugins will be enabled. You can disable them later.'}
        </p>
      </div>
