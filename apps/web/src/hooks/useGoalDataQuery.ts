...
import { createPluginContext } from '@goal-chaser/sdk'
...
async function fetchPluginData(
  ...
  for (const addonId of enabledAddons) {
    ...
    if (plugin?.dataProvider) {
      const context = createPluginContext({ userId, goalId, pluginId: addonId, db: getDb() })
  ...
  const updatePluginDataMutation = useMutation({
    mutationFn: async ({
      ...
      if (pluginId === 'calendar') {
        ...
        if (plugin?.dataProvider) {
          const context = createPluginContext({ userId, goalId, pluginId, db: getDb() })
