...
export async function generateTasks(payload: unknown): Promise<GenerateTasksResult> {
  if (!process.env.OPENAI_API_KEY) {
    throw new Error('OpenAI API key not configured')
  }

  const body = payload as GenerateTasksPayload
  const {
    planText,
    progressSoFar = [],
    completedTasks = [],
  ...
  if (!planText || !date || !goalStartDate) {
    throw new Error('planText, date, goalStartDate required')
  }
  ...
  if (unsummarized && unsummarized.tasks.length > 0) {
    try {
      const data = await summarizeProgress({
  ...
  const progressText =
    progressSoFar.length === 0 && !newSummary
      ...
      : [
          ...progressSoFar.map((s) => `[${s.periodLabel}] ${s.summary}`),
...
Return JSON: { "tasks": [ { "title": "...", "phase": "Phase 1" or "Phase 2" etc., "howToAchieve": "1-2 sentences on how to achieve this task" } ] }`
  ...
  try {
    ...
    console.error('[generateTasks] Parse failed:', raw)
  }
  ...
  const response: GenerateTasksResult = {
    newSummary,
    tasks,
    usage: genUsage,
    totalUsage: {
