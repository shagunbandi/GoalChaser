...
import { usePluginPage, LoadingState, NotFoundState, ContentLoader } from '@goal-chaser/sdk'
...
export default function ExecutiveGoalPage({
  context,
  params,
  year,
  month,
  ...
  const loadGoals = useCallback(async () => {
    ...
    const goals = await loadExecutiveGoals(context)
  ...
  const handleAddExecutiveGoal = async (input: ExecutiveGoalInput) => {
    ...
    const ok = await saveExecutiveGoal(context, goal)
    if (ok) await loadGoals()
  }

  const handleUpdateExecutiveGoal = async (updated: ExecutiveGoal) => {
    const ok = await saveExecutiveGoal(context, updated)
    if (ok) await loadGoals()
  }

  const handleDeleteExecutiveGoal = async (executiveGoalId: string) => {
    const ok = await deleteExecutiveGoal(context, executiveGoalId)
    if (ok) await loadGoals()
  }

  const loadAllPlansForGoal = () => loadExecutiveGoals(context)
