...
interface ExecutiveGoalDetailContext {
  ...
  onAddExecutiveGoal?: (
    executiveGoal: ExecutiveGoalInput,
  ) => void | Promise<void>
  selectedDate?: string
            ...
            <div className="mt-2 space-y-2">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="px-3 py-1 rounded-lg text-xs font-medium bg-[#8B5CF6] hover:bg-[#7C3AED] text-white transition-colors"
                >
                  Save
                </button>
                <button
                  ...
                  className="px-3 py-1 rounded-lg text-xs font-medium bg-white/10 hover:bg-white/15 text-white/70 transition-colors"
                >
                  Cancel
                </button>
              </div>
...
function ExecutiveGoalPlanCard({
  ...
  const [lastPromptsUsed, setLastPromptsUsed] = useState<{
    system: string
    user: string
  } | null>(null)
  ...
  const runGenerateTasks = async () => {
    ...
    try {
      ...
      const completedForGoal = tasks.filter(
        (t) =>
          t.parentExecutiveGoalId === plan.id &&
          t.completed === true &&
      ...
      if ((data.newSummary || data.totalUsage) && onEdit) {
        ...
        if (data.totalUsage) {
          ...
          updates.aiUsage = {
            totalPromptTokens:
              (prev?.totalPromptTokens ?? 0) + totalUsage.promptTokens,
            totalCompletionTokens:
              (prev?.totalCompletionTokens ?? 0) + totalUsage.completionTokens,
            totalTokens: (prev?.totalTokens ?? 0) + totalUsage.totalTokens,
            estimatedCostUsd:
              (prev?.estimatedCostUsd ?? 0) + totalUsage.estimatedCostUsd,
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-white/50">
              {goalsForDay.length} goal{goalsForDay.length !== 1 ? 's' : ''},{' '}
              {tasksForDay.length} task{tasksForDay.length !== 1 ? 's' : ''} on
              this day
            </p>
          </div>
        </div>
