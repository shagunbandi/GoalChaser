...
export interface PluginChatConfig<TInput = unknown, TResult = unknown> {
   ...
   * Ignored when pluginAction is set (uses /api/plugin-action instead).
   */
  apiEndpoint: string

  /**
   * When set, chat requests are sent to POST /api/plugin-action with
   * { pluginId, action, payload: { messages, systemPrompt, todayISO, context } }.
   */
