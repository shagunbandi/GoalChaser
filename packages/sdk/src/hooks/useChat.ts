...
export interface UseChatOptions<TResult = unknown> {
  /** API endpoint to send messages to (ignored when pluginAction is set) */
  apiEndpoint: string
  /** When set, POST to /api/plugin-action with pluginId, action, and payload */
  pluginAction?: { pluginId: string; action: string }
...
export function useChat<TResult = unknown>(
  ...
  const usePluginAction = Boolean(pluginAction)
  const pluginId = pluginAction?.pluginId
  const action = pluginAction?.action
  ...
  const sendMessage = useCallback(async () => {
    ...
    try {
      ...
      const body = usePluginAction && pluginId && action
        ? JSON.stringify({
            pluginId,
            action,
            payload: {
      ...
      const res = await fetch(usePluginAction ? '/api/plugin-action' : apiEndpoint, {
        method: 'POST',
      ...
      const data = (await res.json()) as ChatResponse<TResult>
  ...
  }, [input, isLoading, messages, todayISO, context, apiEndpoint, usePluginAction, pluginId, action, buildSystemPrompt, parseResponse])
