    ...
    <Modal open={open} title={`Generate tasks for ${label}`} onClose={onClose}>
      <div className="space-y-4">
        ...
        {!isLoading && !error && suggestedTasks.length === 0 && (
          <p className="text-sm text-white/50">No tasks suggested. Try regenerating.</p>
        )}

        {!isLoading && suggestedTasks.length > 0 && (
          <>
            <p className="text-xs text-white/50">Select tasks to add:</p>
              ...
              {suggestedTasks.map((t, i) => (
                <li key={i} className="flex items-center gap-2">
                  <input
                    ...
                    onChange={() => toggle(i)}
                    className="rounded border-white/30 bg-white/5 text-[#8B5CF6] focus:ring-[#8B5CF6]"
                  />
