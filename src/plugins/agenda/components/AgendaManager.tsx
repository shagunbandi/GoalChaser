...
import type { AgendaItem, RepeatType } from '@/plugins/agenda/types'
...
interface AgendaManagerProps {
  ...
  onUpdateDetails: (iso: string, details: any) => Promise<void>
  onStatus?: (status: {
    text: string
...
export function AgendaManager({
      ...
      if (editingAgendaId) {
        if (editingRecurrenceId) {
          ...
          const results = await Promise.all(
            datesToUpdate.map(async (iso) => {
              ...
              const withoutSeries = isPast
                ? items
                : items.filter((i: AgendaItem) => i.recurrenceId !== recId)
          ...
          const results = await Promise.all(
            datesToUpdate.map(async (iso) => {
              const items = dayDetails[iso]?.agendaItems || []
              const updatedItems = items.map((item: AgendaItem) => {
  ...
  const handleDeleteAgendaItem = (iso: string, id: string) => {
    try {
      const existingItems = dayDetails[iso]?.agendaItems || []
      const filtered = existingItems.filter((item: AgendaItem) => item.id !== id)
  ...
  const handleDeleteSeries = async (recurrenceId: string) => {
    try {
      ...
      for (const [iso, details] of Object.entries(dayDetails)) {
        ...
        if (items.some((item: AgendaItem) => item.recurrenceId === recurrenceId)) {
  ...
  const toggleAgendaCompletion = (
    ...
    try {
      const existingItems = dayDetails[iso]?.agendaItems || []
      const updatedItems = existingItems.map((item: AgendaItem) =>
      ...
      if (completed) {
        const subjectsToAttach =
          existingItems.find((i: AgendaItem) => i.id === id)?.subjects || []
        ...
        subjectsToAttach.forEach((subj: string) => {
