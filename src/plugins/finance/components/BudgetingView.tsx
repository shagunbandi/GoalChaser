...
export function BudgetingView({
  ...
  const getBudgetRemaining = (budget: BudgetPlan) => {
    const budgetExpenses = allExpenses.filter((e: Expense) => e.budgetId === budget.id)
    const budgetIncome = allIncome.filter((i: Income) => i.budgetId === budget.id)
  ...
  const updateBudgetFromNow = async (plan: BudgetPlan) => {
    ...
    for (let i = 0; i < monthsToGenerate; i++) {
      ...
      const budgetData: BudgetPlan = {
        ...
        name: budgetName,
        income: plan.income,
        categories: plan.categories.map((c: BudgetCategory) => ({ ...c })),
        startDate: startDateISO,
        endDate: endDateISO,
        note: plan.note,
        isRecurring: true,
  ...
  const removeExpense = async (expense: Expense) => {
    ...
    await onUpdateDay(expense.date, {
      expenses: existing.filter((e: Expense) => e.id !== expense.id),
    })
  }
  ...
  const removeIncome = async (income: Income) => {
    ...
    await onUpdateDay(income.date, {
      income: existing.filter((i: Income) => i.id !== income.id),
    })
  }
      ...
      modal: {
        getSections: (date: string) => {
          ...
          const budgetExpenses = activeBudget
            ? allExpenses.filter((e: Expense) => e.budgetId === activeBudget.id)
            : []
          const budgetIncome = activeBudget
            ? allIncome.filter((i: Income) => i.budgetId === activeBudget.id)
            : []
              ...
              <div className="space-y-2">
                {budgets.map((p) => (
                  <BudgetCard
                    key={p.id}
                    budget={p}
                    expenses={allExpenses.filter((e: Expense) => e.budgetId === p.id)}
